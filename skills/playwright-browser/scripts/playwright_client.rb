...
class PlaywrightClient
  ...
  def initialize(host: 'localhost', port: 3001, logger: nil)
    @host = host
    @port = port
  ...
  def verify_connection
    ...
    begin
      ...
      @logger.info("Connected to Playwright API Server: #{response}")
    rescue => e
      ...
      raise ConnectionError, "Failed to connect to Playwright API Server at #{@base_url}: #{e.message}"
    end
  end
