...
3. If not found, create default configuration by prompting user for:
   - Project name
   - Default base branch (typically "develop")
...
6. **Base branch verification**: Confirm on develop or prepare to switch
...
- `BASE_BRANCH`: Default base branch (default: "develop")
...
1. **User-provided task description**: If user specifies task in skill invocation
2. **User-specified GitHub Issue**: If user provides issue number
3. **Highest priority open issue**: Default behavior - fetch from GitHub
...
2. Switch to base branch (develop) if not already there
...
❌ Cannot start new task from protected branch: main
   New tasks must be started from develop branch
```
**Action**: Switch to develop manually or let skill switch automatically
